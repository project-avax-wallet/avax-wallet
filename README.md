...
✏ Rename `.env.local.example` to `.env` in the main folder and provide your `appId` and `serverUrl` from Moralis (check with 0xWout)
Example:
...
# 📒 Notes
We enforce the user to use the avalanche testnetwork (but Moralis only supports Mainnet server)
